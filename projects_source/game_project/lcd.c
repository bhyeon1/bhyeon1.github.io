...
#if 1	
	Lcd_WR_REG(0xB8); // Backlignt Control 1
	Lcd_WR_DATA(0x0f); // default => 0x06
	Lcd_WR_REG(0xBA); // Backlignt Control 3
	Lcd_WR_DATA(0x0f); // default => 0x06	
	Lcd_WR_REG(0xBB); // Backlignt Control 4
	Lcd_WR_DATA(0xff); // default => 0x06	
	Lcd_WR_REG(0xBE); // Backlignt Control 7
	Lcd_WR_DATA(0xFF); // default => 0x0F
#endif
...
void draw_image(int xs, int ys, tImage * d)
{
	...
	for(i=0; i < (xe - xs + 1) * (ye - ys + 1); i++)
	{
		_SPI1_Write_Byte(d->data[i] >> 8);
		_SPI1_Write_Byte(d->data[i] & 0xff);
	}
