...
{
	msTicks++;
  if (bgm_timer > 0 && --bgm_timer == 0) {
            /* 다음 음 연주 */
            Note curr = bgmTheme[bgm_idx];                  // 현재 인덱스의 음
            TIM3_Out_Freq_Generation( SemitoneToFreq(curr.tone) );
            bgm_timer = curr.dur;                           // 지속 시간 설정
            bgm_idx   = (bgm_idx + 1) % THEME_LEN;          // 인덱스 순환
         }
}
...
extern volatile int vel_y;
extern volatile int request_jump;
extern volatile int request_stop;

 void EXTI15_10_IRQHandler(void)
 {
  if (Macro_Check_Bit_Set(EXTI->PR, 13))
  {
    if (!is_jumping && !request_jump) {
      request_jump = 1;
    }
  }

  if (Macro_Check_Bit_Set(EXTI->PR, 14))
  {
    request_stop = 1;
  }
   EXTI->PR = 0x3<<13;
