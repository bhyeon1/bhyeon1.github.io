...
function [fft_out, module2_out] = fft_fixed_stu(fft_mode, fft_in)
 ...
 fp=fopen('ran_re_5.txt','w');
 for nn=1:512
    fprintf(fp, '%d\n', real(din(nn)) );
 end

 fp=fopen('ran_im_5.txt','w');
 for nn=1:512
    fprintf(fp, '%d\n', imag(din(nn)) );
 end
 ...
 fp=fopen('fft_ran_matlab_00.txt','w');
 for nn=1:512
    fprintf(fp, 'bfly00(%d)=%d+j%d\n', nn, real(bfly00(nn)) ,imag(bfly00(nn)) );
 end
 fclose(fp);
...
% fclose(fp_1);
 ...
 fp=fopen('fft_ran_matlab_01.txt','w');
 for nn=1:512
    fprintf(fp, 'bfly01(%d)=%d+j%d\n', nn, real(bfly01(nn)) ,imag(bfly01(nn)) );
 end
 fclose(fp);

 % step0_2
 % m1: bfly01 - 13bit; bfly02_tmp - 14bit; pre_bfly02 - 14bit; bfly02 - 11bit
 for kk=1:4
  for nn=1:64
...
% fp=fopen('fft_02_test.txt','w');
% for nn=1:512
%    fprintf(fp, 'bfly02(%d)=%d+j%d\n', nn, real(bfly02_tmp(nn)) ,imag(bfly02_tmp(nn)) );
% end
% fclose(fp);

 for ii=1:512
 	%bfly02_tmp = sat(bfly02_tmp, 13); % Saturatin (13 bit)
 	bfly02_tmp_1(ii) = sat(bfly02_tmp(ii), 13); % Saturatin (13 bit)
 end

 
% fp=fopen('fft_02_test_1.txt','w');
% for nn=1:512
%    fprintf(fp, 'bfly02(%d)=%d+j%d\n', nn, real(bfly02_tmp_1(nn)) ,imag(bfly02_tmp_1(nn)) );
% end
% fclose(fp);
 ...
 for nn=1:512
	...
	pre_bfly02(nn) = bfly02_tmp_1(nn)*twf_m0(nn); % (14bit(13+1) * 9bit = 23 bit) 
 end


 fp=fopen('fft_ran_matlab_02.txt','w');
 for nn=1:512
...
% fclose(fp_2);
 ...
 fp=fopen('matlab_module_0.txt','w');
 for nn=1:512
 ...
 fp=fopen('fft_ran_matlab_03.txt','w');
 for nn=1:512
    fprintf(fp, 'bfly10(%d)=%d+j%d\n', nn, real(bfly10(nn)) ,imag(bfly10(nn)) );
 end
 fclose(fp);
...
% fclose(fp_3);
 ...
 fp=fopen('fft_ran_matlab_04.txt','w');
 for nn=1:512
    fprintf(fp, 'bfly11(%d)=%d+j%d\n', nn, real(bfly11(nn)) ,imag(bfly11(nn)) );
 end
 fclose(fp);
...
% fp=fopen('aaaaaaaaa.txt','w');
% for nn=1:512
%    fprintf(fp, 'bfly12(%d)=%d+j%d\n', nn, real(bfly12_tmp(nn)) ,imag(bfly12_tmp(nn)) );
% end
% fclose(fp);
...
% fp=fopen('twiddle_12.txt','w');
% for nn=1:64
%    fprintf(fp, 'twd12(%d)=%d+j%d\n', nn, real(twf_m1(nn)) ,imag(twf_m1(nn)) );
% end
% fclose(fp);
...
% fp=fopen('fft_matlab_module1.txt','w');
% for nn=1:512
%    fprintf(fp, 'bfly12(%d)=%d+j%d\n', nn, real(pre_bfly12(nn)) ,imag(pre_bfly12(nn)) );
% end
% fclose(fp);
...
% fclose(fp_4);
 ...
 fp=fopen('fft_ran_matlab_05.txt','w');
 for nn=1:512
 ...
 fp=fopen('twd20.txt','w');
 for nn=1:8
    fprintf(fp, 'twd(%d)=%d+j%d\n', nn, real(fac8_0(ceil(nn/2))), imag(fac8_0(ceil(nn/2))) );
 end
 fclose(fp);

 fp=fopen('fft_ran_matlab_06.txt','w');
 for nn=1:512
    fprintf(fp, 'bfly20(%d)=%d+j%d\n', nn, real(bfly20(nn)) ,imag(bfly20(nn)) );
 end
 fclose(fp);
...
% fclose(fp_5);
 ...
 fp=fopen('fft_ran_matlab_07.txt','w');
 for nn=1:512
    fprintf(fp, 'bfly21(%d)=%d+j%d\n', nn, real(bfly21(nn)) ,imag(bfly21(nn)) );
 end
 fclose(fp);
 ...
 fp=fopen('fft_ran_matlab_08.txt','w');
 for nn=1:512
    fprintf(fp, 'bfly22(%d)=%d+j%d\n', nn, real(bfly22_tmp(nn)) ,imag(bfly22_tmp(nn)) );
 end
 fclose(fp);
 ...
 fp=fopen('fft_ran_matlab_09.txt','w');
 for nn=1:512
 ...
 fp=fopen('ran_final_5.txt','w');
 for nn=1:512
    fprintf(fp, 'dout(%d)=%d+j%d\n', nn, real(dout(nn)) ,imag(dout(nn)) );
 end
 fclose(fp);
